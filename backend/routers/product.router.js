...
router.get("/", async () => {
  response(res, async () => {
    ...
    let products = await Product.find({
      // Açıklama burada sadece bir adet sorgu yaptığımız için or kullanmaya gerek yok çünkü sadece name key değerini istiyoruz
...
router.post("/removeImageByProductIdAndIndex", async (req, res) => {
  response(res, async () => {
    ...
    if (product.imageUrls.length == 1) {
      ...
      // that's means, start from index and delete 1 object
      await Product.findByIdAndUpdate(_id, product);
      fs.unlink(image.path, () => {});
